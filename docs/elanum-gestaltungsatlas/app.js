...
const frames = $$('[data-resonance]').map((container, index) => {
  ...
  return {container, svg, gradient, a,b,bridge,nodeA,nodeB,ink, id:index, visible:false, labelA:$('[data-person=a]',container),labelB:$('[data-person=b]',container)};
});
...
function updateSignature(){
  ...
  const mode={master:'Spektrum',ink:'Ink',inverse:'Invers'}[signatureMode];
...
[['Air','#F8F9F7'],['Surface','#FFFFFF'],['Ink','#16363B'],['Paper','#F2ECDD']].forEach(([name,color])=>{const button=colorButton(name,color,'swatch');button.innerHTML=`<strong>${name}</strong><code>${color}</code>`;$('#basePalette').append(button);});
